...
// ── Project Settings (Buildspec & Slack Channel) ─────────────────────────
...
// ── Project Slack Channel Management ─────────────────────────────────────
async function loadProjectSlackStatus() {
  if (!window._activeProjectId) {
    try {
      ...
      if (pRes.ok && pRes.projects && pRes.projects.length > 0) {
        ...
        window._activeProjectId = active.id;
      }
    } catch (_) {}
  }
  if (!window._activeProjectId) return;
  ...
  try {
    ...
    if (res.ok && res.project) {
      ...
      if (labelEl) labelEl.textContent = p.name || window._activeProjectId;
    ...
    console.error("Failed to load project Slack status:", e);
  }
}
...
async function provisionProjectSlackChannel() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      showMsg(msgEl, '✔ Slack channel #' + ch + ' provisioned & members synced successfully!', false);
      loadProjectSlackStatus();
    } else {
