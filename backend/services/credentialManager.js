...
async function storeCredential(username, { provider, repositoryId, credentialType, token, label, meta, expiresAt }) {
  ...
  const [existing] = await pool.query(
    `SELECT id FROM repo_credentials WHERE username = ? AND provider = ? AND (repository_id = ? OR repository_id IS NULL)`,
    [username, provider, repositoryId || null]
  );
  ...
  if (existing.length > 0) {
    ...
    await pool.query(
      `UPDATE repo_credentials
       SET encrypted_token = ?, token_iv = ?, token_tag = ?, label = ?, meta = ?, expires_at = ?
       WHERE id = ?`,
      [encrypted, iv, tag, label || null, metaStr, expiresAt || null, id]
    );
    return { id, username, provider, credentialType: credentialType || "pat", label, repositoryId };
  } else {
    ...
    await pool.query(
      `INSERT INTO repo_credentials
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, username, repositoryId || null, provider, credentialType || "pat", encrypted, iv, tag, label || null, metaStr, expiresAt || null]
    );
    return { id, username, provider, credentialType: credentialType || "pat", label, repositoryId };
  }
}
...
async function deleteCredential(credentialId, username) {
  const [rows] = await pool.query(
    "SELECT id FROM repo_credentials WHERE id = ? AND username = ?",
...
async function testCredential(credentialId, username) {
  const [rows] = await pool.query(
    "SELECT * FROM repo_credentials WHERE id = ? AND username = ?",
    [credentialId, username]
  );
...
async function getCredentialByProvider(username, provider) {
  const [rows] = await pool.query(
    `SELECT * FROM repo_credentials WHERE username = ? AND provider = ? ORDER BY created_at DESC LIMIT 1`,
    [username, provider]
  );
  if (!rows.length) return null;
  const row = rows[0];
...
async function deleteCredentialByProvider(username, provider) {
  await pool.query("DELETE FROM repo_credentials WHERE username = ? AND provider = ?", [username, provider]);
  return true;
}
...
async function resolveGithubToken(username) {
  if (username) {
    try {
      const tok = await getCredential(username, "github");
      if (tok) return tok;
    } catch (_) {}
  }
  ...
  try {
    const [rows] = await pool.query(
      `SELECT encrypted_token, token_iv, token_tag FROM repo_credentials WHERE provider = 'github' ORDER BY created_at ASC LIMIT 1`
    );
    if (rows.length > 0) {
