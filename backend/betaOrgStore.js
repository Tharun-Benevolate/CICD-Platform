...
async function ensureSchema() {
  const [rows] = await pool.query(
    `SELECT COUNT(*) AS cnt FROM information_schema.columns
     WHERE table_schema = DATABASE() AND table_name = 'beta_orgs' AND column_name = 'org_domain'`
  );
  if (rows[0].cnt === 0) {
    await pool.query(
      "ALTER TABLE beta_orgs ADD COLUMN org_domain VARCHAR(255) UNIQUE AFTER org_name"
    );
    console.log("[betaOrgStore] migrated beta_orgs: added org_domain column");
  }
}
